  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m)
  {
    ...
    m = min(n - tot, BSIZE - off%BSIZE);
     ...
     if(ip->type == T_CHECKED)
     {
      ...
      int current_checksum = check_checksum((char*)bp->data, addr);
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m)
  {
    ...
    if(ip->type == T_CHECKED)
    {    
      uint checksum = create_checksum((char*)bp->data);
      int blockindex = -1;
      for(int i = 0; i < NDIRECT; i++)
      {
        if((ip->addrs[i] & 0x00ffffff) == block)
...
uint create_checksum(char* data)
{
  uint checksum = 0;
...
int check_checksum(char* data, uint addr)
{
  uint currcksm = create_checksum(data);
